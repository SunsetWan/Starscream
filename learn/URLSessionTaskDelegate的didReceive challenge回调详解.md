...
### Q: TLS 1.2 的静态 RSA 密钥交换为什么没有前向保密？

#### 先回顾：TLS 1.2 支持两类密钥交换算法

| 密钥交换方式 | 有无前向保密 | TLS 1.3 是否保留 |
|------------|------------|-----------------|
| 静态 RSA | ❌ 无 | ❌ **已禁止** |
| DHE / ECDHE（临时迪菲-赫尔曼） | ✅ 有 | ✅ **强制使用** |

关键区别在于：**session key 的生成过程中，服务器的长期私钥扮演了什么角色？**

#### 静态 RSA 密钥交换：私钥直接参与解密 → 无前向保密
...
生成随机 premaster secret
用服务器长期公钥加密
              ──────────────────────▶
                                       用服务器长期私钥解密
                                       得到 premaster secret

双方计算：session key = PRF(premaster secret, client random, server random)
```

**问题出在哪？** premaster secret 是用服务器的**长期公钥**加密的。这意味着：

```
攻击者的操作：
  ① 今天：录制所有加密流量（包含加密的 premaster secret）
  ② 明年：服务器长期私钥泄露（被黑客入侵、员工离职、硬件报废未清除…）
  ③ 用泄露的私钥解密当初录制的 premaster secret
  ④ 用 premaster secret + client random + server random 算出 session key
  ⑤ 用 session key 解密所有历史流量

  一把私钥泄露 → 所有历史通信全部暴露 ❌
```

#### ECDHE 密钥交换：私钥只用于签名 → 有前向保密

```
客户端                                           服务器

生成临时密钥对：                                   生成临时密钥对：
  临时私钥 a（仅本次连接使用）                       临时私钥 b（仅本次连接使用）
  临时公钥 A = a×G                                 临时公钥 B = b×G
...
客户端计算：shared secret = a × B                服务器计算：shared secret = b × A
双方计算：session key = PRF(shared secret, client random, server random)

连接结束后：临时私钥 a 和 b 被销毁 🗑️
```
...
```
攻击者的操作：
  ① 今天：录制所有加密流量（包含临时公钥 A 和 B）
  ② 明年：服务器长期私钥泄露
  ③ 能做什么？
     - 用长期私钥伪造签名？✅ 可以，但只能用于冒充服务器发起新的攻击
     - 解密历史流量？❌ 不行！
       因为 session key 是从 shared secret = a×b×G 算出的
       而 a 和 b（临时私钥）早已销毁
       只有 A 和 B（临时公钥）在网络上传输过
       从 A 和 B 反推 a 或 b → 椭圆曲线离散对数问题 → 计算上不可行
```
...
| | 静态 RSA | ECDHE |
|---|---|---|
| **服务器长期私钥的角色** | **解密** premaster secret | **签名**临时公钥（证明身份） |
| **session key 依赖什么** | premaster secret（用长期公钥加密传输） | 临时 DH 共享密钥（从未传输） |
| **临时密钥** | 无 | 每次连接生成新的临时密钥对，用完即销毁 |
| **长期私钥泄露的后果** | 所有历史流量可被解密 | 只能冒充服务器发起新攻击，历史流量安全 |
| **一句话总结** | 私钥是"万能钥匙"——解锁所有连接 | 私钥只是"身份证"——证明身份，不解锁数据 |

> **前向保密的本质**：每次连接的加密密钥都是**独立的、临时的、用完即销毁的**。没有任何一把"永久密钥"能解开所有连接。这就是 TLS 1.3 强制 ECDHE、禁止静态 RSA 的原因。
